 ...
 */
async function crewMatch(request: APIRequestContext): Promise<{ matchId: string, playerId: number } | null> {
  const playersResponse = await request.get('/api/gameon/lol/summoner?archived=false')
  ...
  for (const player of players.slice(0, 5)) {
    const matches = await request.get(`/api/gameon/lol/match/player/${player.id}?page=1&size=1&rankedOnly=false`)
...
async function firstCrewPlayer(request: APIRequestContext): Promise<CrewPlayer | null> {
...
test.describe('match detail page', () => {
  ...
  test('renders a real match with both teams and its tabs', async ({ page, request }) => {
    ...
    const response = await page.goto(`/game/${match!.matchId}/${match!.playerId}`)
    ...
    for (const tab of ["Vue d'ensemble", 'Film de la partie', 'Performance', 'Données brutes']) {
...
test.describe('summoner profile', () => {
  ...
  test('is server-rendered, so crawlers receive the player name', async ({ page, request }) => {
    ...
    await expect(page).toHaveTitle(new RegExp(player!.nickname.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
  })
})
