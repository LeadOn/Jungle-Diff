...
- Dependencies refreshed to latest on 2026-09-07 (`@iconify-json/lucide` 1.2.130, `@types/node` 26.5.0, `eslint` 10.10.0, `vue` 3.5.42; everything else already on latest: `nuxt` 4.5.2, `@nuxtjs/i18n` v10, `@pinia/nuxt` v1 / `pinia` v4, `@nuxt/eslint` v1, `flowbite-vue` v0.4, `oidc-client-ts` v3.5, `tailwindcss`/`@tailwindcss/vite` v4.3, `vue-tsc` v3.3). `typescript` remains intentionally pinned to `^6.0.3` rather than the v7 "Corsa" native-compiler line — `typescript-eslint` (pulled in by `@nuxt/eslint-config`, latest 8.69.0) still declares `typescript >=4.8.4 <6.1.0` and crashes under v7; re-check that peer range before bumping. Build, `vue-tsc --noEmit`, and `eslint .` all verified after the refresh (the remaining ESLint errors/warnings are pre-existing code-quality debt, not upgrade regressions).
