...
- **Dependencies:** Refreshed to latest on 2026-09-07 (`@iconify-json/lucide` 1.2.130, `@types/node` 26.5.0, `eslint` 10.10.0, `vue` 3.5.42; the rest were already current: `nuxt` 4.5.2, `@nuxtjs/i18n` v10, `@pinia/nuxt` v1 / `pinia` v4, `@nuxt/eslint` v1, `flowbite-vue` v0.4, `oidc-client-ts` v3.5, `tailwindcss`/`@tailwindcss/vite` v4.3, `vue-tsc` v3.3). `typescript` stays pinned to `^6.0.3` — the v7 "Corsa" native-compiler line still breaks `typescript-eslint` (via `@nuxt/eslint-config`), whose latest release (8.69.0) requires `typescript >=4.8.4 <6.1.0`. `npm audit` reports 0 vulnerabilities; build and `vue-tsc --noEmit` confirmed working post-update.
